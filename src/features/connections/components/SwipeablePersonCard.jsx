            ...
            <div className="flex flex-wrap gap-1.5 sm:gap-2 mb-2">
              {person.matchReasons.map(reason => {
                ...
                return (
                  <span
                    key={reason}
                    className={`inline-flex items-center gap-1 px-2.5 py-1 text-[10px] font-bold rounded-full border ${isHighlightedReason
                      ? 'bg-indigo-50 text-indigo-700 border-indigo-100'
                      : 'bg-slate-100 text-slate-700 border-slate-200'
                      }`}
                  >
                    {ReasonIcon && <ReasonIcon className={`w-3 h-3 ${isHighlightedReason ? 'text-indigo-600' : 'text-slate-500'}`} />}
                    {reason}
                  </span>
                );
              })}
